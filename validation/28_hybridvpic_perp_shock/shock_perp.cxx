...
begin_initialization {
  ...
  double Vd_Va    = 4.0;             // Alfven Mach number
  double Ti_Te    = 1.0;            // Ion temperature / electron temperature
  double theta    = 90.0*M_PI/180.0;  // Shock normal/B field angle
  ...
  double taui    = 30;    // Simulation run time in wci^-1.
  double quota   = 23.5;   // run quota in hours
  double quota_sec = quota*3600;  // Run quota in seconds
  
  double Lx    = 200.0*di;    // size of box in x dimension
  double Ly    = 1.0*di;    // size of box in y dimension
  double Lz    = 8.0*di;     // size of box in z dimension
...
begin_diagnostics {
  ...
  // --- code-to-code comparison: dump the Bz(x) profile at the final step ---
  // (1D perpendicular shock, 1 rank; Bz/n frozen-in => compression and overshoot
  //  both follow from Bz(x). x is in cells; multiply by hx for d_i.)
  if ( step() == num_step ) {
    FileIO fpbz;
    fpbz.open("bz_profile.txt", io_write);
    for (int i = 1; i <= grid->nx; i++) {
      double bzsum = 0;
      for (int k = 1; k <= grid->nz; k++) bzsum += field(i, 1, k).cbz;
      fpbz.print("%d %.10e\n", i, bzsum / grid->nz);
    }
    fpbz.close();
  }
  ...
  if(step()==0) {
    ...
    // global_header disabled for the code-to-code run (binary-output machinery
    // aborts in this minimal 1-rank config; we only need the ASCII Bz dump above).
  ...
  // field_dump (binary .gda output) disabled — replaced by the ASCII Bz dump above.
