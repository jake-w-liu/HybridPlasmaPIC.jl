...
function case_06_boundary_loading_kdv_smoothing(artifact_dir::AbstractString)
    ...
    quiet_error =
        maximum(abs(sum(ps_quiet.v[c]) / nparticles(ps_quiet) - (0.1, -0.2, 0.3)[c]) for c = 1:3)
    ...
    nextid = Ref(UInt64(1))
    ninj =
        inject_face_1d!(ps_inj, rng, 0.0, +1, 1.0, 2.0, 0.0, (0.0, 0.0), 0.3, 1.0, 0.5, acc, nextid)
    injection_error =
        ninj == 4 &&
        nparticles(ps_inj) == 4 &&
        all(==(2.0), ps_inj.v[1]) &&
    ...
    rows = (
        (
            "absorbing_boundary_compaction_error",
            absorbing_error,
            0.0,
            "absolute",
            absorbing_error,
        ...
        (
            "density_weight_total_abs_error",
            sum(ps_quiet.weight),
            1.25 * prod(gload.L),
            "absolute",
            weight_error,
        ...
        (
            "cold_flux_per_density_abs_error",
            flux_per_density(2.0, 0.0),
            2.0,
            "absolute",
            flux_error,
            0.0,
        ...
        (
            "cold_flux_speed_abs_error",
            flux_speed(MersenneTwister(6), 2.0, 0.0),
            2.0,
            "absolute",
            flux_speed_error,
            0.0,
        ...
        (
            "binomial_smoothing_single_mode_error",
            smoothing_error,
            0.0,
            "absolute",
            smoothing_error,
