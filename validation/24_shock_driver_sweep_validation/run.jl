...
function case_24_shock_driver_sweep_validation(artifact_dir::AbstractString)
    ...
    direct3_error =
        all(isfinite, (direct3.n2, direct3.frozen_ratio, direct3.maxdivB, direct3.mean_xs, direct3.M_real)) ? 0.0 : 1.0
    ...
    reflected_error =
        all(0.0 <= r.reflected_fraction <= 1.0 && all(isfinite, (r.n2, r.Bz2, r.Vs, r.M_real)) for r in sweep) ? 0.0 : 1.0
    ...
    compare_dims_error =
        isfinite(dims.oned.n2) && isfinite(dims.oned.frozen_ratio) &&
        isfinite(dims.threed.n2) && isfinite(dims.threed.frozen_ratio) ? 0.0 : 1.0

    artifact = joinpath(artifact_dir, "24_shock_driver_sweep_validation.csv")
    rows = (
        ("run_perp_shock_direct_frozen_abs_error", direct.frozen_ratio, 1.0, "absolute", direct_frozen_error, 0.15),
        ("run_perp_shock_direct_contract_error", direct_contract_error, 0.0, "absolute", direct_contract_error, 0.0),
        ("reproduce_established_shock_oracle_error", established_error, 0.0, "absolute", established_error, 0.0),
        ...
        ("run_perp_shock3d_direct_finite_error", direct3_error, 0.0, "absolute", direct3_error, 0.0),
        ("four_spacecraft_traces_one_step_contract_error", trace_driver_error, 0.0, "absolute", trace_driver_error, 0.0),
        ("perp_vs_mach_sweep_max_abs_error", driver_equivalence_error, 0.0, "absolute", driver_equivalence_error, 0.0),
        ("supercritical_frozen_in_max_abs_error", maximum(abs(r.frozen_ratio - 1.0) for r in sweep), 0.0, "absolute", frozen_error, 0.08),
        ("supercritical_compression_band_violation", compression_error, 0.0, "margin", compression_error, 0.0),
        ("supercritical_mass_speed_relative_error", speed_error, 0.0, "relative", speed_error, 1e-12),
        ("supercritical_reflected_fraction_contract_error", reflected_error, 0.0, "absolute", reflected_error, 0.0),
        ("shock_convergence_rel_max", cv.rel_max, 0.0, "absolute", convergence_error, 0.15),
        ("shock_convergence_flag_error", convergence_flag_error, 0.0, "absolute", convergence_flag_error, 0.0),
        ("production_3d_restart_bitmatch_error", production_restart_error, 0.0, "absolute", production_restart_error, 0.0),
        ("production_3d_finite_diagnostics_error", production_finite_error, 0.0, "absolute", production_finite_error, 0.0),
        ("shock_campaign_3d_driver_contract_error", campaign_error, 0.0, "absolute", campaign_error, 0.0),
        ("compare_dims_shock_driver_contract_error", compare_dims_error, 0.0, "absolute", compare_dims_error, 0.0),
    )
