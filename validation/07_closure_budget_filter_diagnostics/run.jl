...
function case_07_closure_budget_filter_diagnostics(artifact_dir::AbstractString)
    ...
    kinetic_ref = sum(ps.weight[p] * 0.5 * ps.m * (ps.v[1][p]^2 + ps.v[2][p]^2 + ps.v[3][p]^2) for p = 1:2)
    magnetic_ref = 0.5 * n * (0.2^2 + (-0.1)^2 + 0.3^2) * prod(g.dx)
    electron_ref = (0.75 / (5 / 3 - 1)) * prod(g.L)
    budget_error = maximum(abs, (budget.kinetic - kinetic_ref, budget.magnetic - magnetic_ref, budget.electron_internal - electron_ref))
    isothermal_budget = energy_budget(ps, B, density, IsothermalElectrons(1.0), g)
    isothermal_error = isnan(isothermal_budget.electron_internal) && isnan(isothermal_budget.total) ? 0.0 : 1.0
    ...
    rows = (
        ("electron_pressure_evolution_max_abs_error", pressure_evolution_error, 0.0, "absolute", pressure_evolution_error, 1e-12),
        ("energy_budget_component_max_abs_error", budget.total, kinetic_ref + magnetic_ref + electron_ref, "absolute", budget_error, 1e-12),
        ("isothermal_budget_nan_contract_error", isothermal_error, 0.0, "absolute", isothermal_error, 0.0),
        ("jdotE_density_integral_max_abs_error", sum(jE) * prod(g.dx), electric_work(J, E, g), "absolute", jdot_error, 1e-12),
        ("resistive_dissipation_abs_error", resistive_dissipation(J, 0.3, g), 0.3 * 5.0 * prod(g.L), "absolute", diss_error, 1e-12),
        ("temperature_parallel_perp_max_abs_error", maximum(Tpar), 2.0, "absolute", temp_error, 1e-12),
        ("pressure_strain_single_mode_max_abs_error", maximum(abs, strain), 4.0, "absolute", strain_error, 1e-12),
        ("power_spectrum_peak_index_error", argmax(power) - 1, 3.0, "absolute", spectrum_error, 1e-12),
    )
