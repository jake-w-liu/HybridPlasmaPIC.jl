...
# sizes, FFTW wisdom round-trip, and the SpectralOperators package re-export.
#
# Oracles are closed-form / definitional and never call the production routine
# under test to derive the expected value.

using HybridPlasmaPIC, Test, LinearAlgebra, FFTW
...
@testset "SpectralOperators package re-exports" begin
    # The namespace exposes the operator API independent of any global state.
    @test SpectralOperators.FourierGrid === HybridPlasmaPIC.FourierGrid
    @test SpectralOperators.deriv! === HybridPlasmaPIC.deriv!
    @test SpectralOperators.gradient! === HybridPlasmaPIC.gradient!
    @test SpectralOperators.divergence! === HybridPlasmaPIC.divergence!
    @test SpectralOperators.curl! === HybridPlasmaPIC.curl!
    @test SpectralOperators.laplacian! === HybridPlasmaPIC.laplacian!
    @test SpectralOperators.project_divfree! === HybridPlasmaPIC.project_divfree!

    # deriv! through the namespace actually computes ∂x cos(2x) = -2 sin(2x).
    T = Float64
    n = 32
